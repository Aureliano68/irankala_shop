...
class shopcart:
    ...
    def add_to_shop_cart(self,product,qty):
        ...
        if product_id not in self.shop_cart:
            self.shop_cart[product_id]={'qty':0,'price':product.price,'price_finall':product.get_price_by_discount()}
    ...
    def __iter__(self):
        ...
        for item in temp.values():
            item['total_price']=item['price_finall']*item['qty']
    ...
    def clac_total_price(self):
        ...
        for item in self.shop_cart.values():
            sum+=int(item['price_finall'])*item['qty']
        return sum
